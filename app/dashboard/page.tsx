...
export default function DashboardPage() {
  ...
  const loadProjectVersions = useCallback(async (projId?: string) => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || ""}/api/projects/${id}/versions`, {
  ...
  const handleRollback = async (versionId: string) => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || ""}/api/projects/${activeProjectId}/versions/${versionId}/rollback`, {
        method: "POST",
        headers: { Authorization: `Bearer ${authToken}` }
      });
      if (res.ok) {
        ...
        // Reload project
        const projectRes = await fetch(`${process.env.NEXT_PUBLIC_API_URL || ""}/api/projects/${activeProjectId}`, {
          headers: { Authorization: `Bearer ${authToken}` }
        });
        if (projectRes.ok) {
